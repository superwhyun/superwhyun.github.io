...
unzd() {
    ...
    if [ -d "$target" ]; then # 압축 풀린 폴더가 존재할 경우, 
        ...
        ditto -xk "$1" "${target##*/}" # -qq outputting 없이 수행
    fi
}
...
for exported_foldername in ${exported_foldername_array[*]}; do
    ...
    # read  meta_subtitle
    echo -n "Enter categories (information, draft, etc): "
    ...
    # echo -e "BEFORE - exported_filename is $exported_filename"
    exported_filename_for_images_path=$(pythonURLEncode "$exported_filename")
    # echo -e "AFT: $exported_filename_for_images_path"

    
    # wtf=$(pythonURLEncode "라즈베리파이에 Mattermost 설치하기 1a9ad3a2d5c246ed9b4e840ee380d0d1")
    # echo -e "WTF: $wtf"
    ...
    # echo -e "exported_file_path is $exported_file_path"
    # echo -e "images_folder_path is $images_folder_path"
